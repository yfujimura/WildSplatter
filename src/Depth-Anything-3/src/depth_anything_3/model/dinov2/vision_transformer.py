...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if self.alt_start != -1:
            ...
            self.appearance_token = nn.Parameter(torch.randn(1, 1, 1, embed_dim))
    ...
    def _get_intermediate_layers_not_chunked(self, x, n=1, export_feat_layers=[], n_ctx=1, **kwargs):
        B, S, _, H, W = x.shape
        with torch.no_grad():
        ...
        for i, blk in enumerate(self.blocks):
            if self.alt_start != -1 and i == self.alt_start:
                pos, pos_nodiff = self._prepare_rope(B, n_ctx, H, W, x.device)
                x = x[:,:n_ctx]
                S = n_ctx
            ...
            if self.alt_start != -1 and i == self.alt_start:
                if kwargs.get("cam_token", None) is not None:
                    ...
                    app_token = self.appearance_token.expand(B, S, -1, -1)
                x[:, :, 0] = cam_token

                #x = torch.cat([x, app_token], dim=2)  # (B,S,N+1,D)

            #if self.alt_start != -1 and i >= self.alt_start:
            #    g_pos = self._append_app_pos(g_pos, app_token, attn_type=None)
            #    l_pos = self._append_app_pos(l_pos, app_token, attn_type=None)

            if self.alt_start != -1 and i >= self.alt_start and i % 2 == 1:
                x = self.process_attention(
                    x, blk, "global", pos=g_pos, attn_mask=kwargs.get("attn_mask", None)
                )
            elif self.alt_start != -1 and i >= self.alt_start and i % 2 == 0:
            ...
            if i in blocks_to_take:
                ...
                #output.append((out_x[:, :, 0], out_x[:, :, :-1]))
                #output.append((out_x[:, :, 0], out_x[:, :, :-1], out_x[:, :, -1]))
    ...
    def _append_app_pos(self, pos, app, attn_type):
        # pos: localなら (B,S,N,2) or (B,S,N,Cpos), globalでも同じ形で来る想定
        # app は 1 token なので pos_app=0 を 1個足す
        if pos is None or app is None:
            return pos
        B,S,N,_ = pos.shape
        pos_app = torch.zeros((B, S, 1, pos.shape[-1]), device=pos.device, dtype=pos.dtype)
        return torch.cat([pos, pos_app], dim=2)  # (B,S,N+1,*)
